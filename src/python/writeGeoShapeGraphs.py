...
def writeGraphFooter(f, id, title, yLabel, series):
  ...
  colors = []
  if 'Geohash (RPT)' in series:
    colors.append('#DD1E2F')
  if 'Geohash (Term)' in series:
    colors.append('#B0A691')
  if 'Quadtree (RPT)' in series:
    colors.append('#EBB035')
  if 'Packed Quadtree (RPT)' in series:
...
def prettyName(approach):
  if approach == 'geohash_term':
    return 'Geohash (Term)'
  elif approach == 'geohash_rpt':
    return 'Geohash (RPT)'
  elif approach == 'quadtree_rpt':
    return 'Quadtree (RPT)'
  elif approach == 'packedquadtree_rpt':
    return 'Packed Quadtree (RPT)'
  elif approach == 'latLonShape':
    return 'LatLonShape'
  else:
...
<p>Below are the results of the Lucene nightly geo shape benchmarks based on the <a href="https://git-wip-us.apache.org/repos/asf/lucene-solr.git">master</a> branch as of that point in time.</p>
<p>This test indexes a 60M shape subset exported from the full (as of 8/9/2018) 510.1M shape <a href="http://openstreetmaps.org">OpenStreetMaps corpus</a>, using five different approaches (GeoHash w/ term strategy, GeoHash, Quadtree, and PackedQuadtree w/ recursive strategies, and vector based LatLonShape), and then tests search and sorting performance of various shapes.  The London boroughs polygons <a href="http://data.london.gov.uk/2011-boundary-files">come from here</a> (33 polygons, average 5.6K vertices).</p>
<p>On each chart, you can click + drag (vertically or horizontally) to zoom in and then shift + drag to move around, and double-click to reset.  Hover over an annotation to see known changes.  Click on a point to see the exact source git revision that was run.</p>
  ...
  for timeStamp, (gitRev, stats, results) in loadAllResults().items():
    ...
    for approach in stats.keys():
      print('before prettyName %s' % approach)
    ...
    for tup in results.keys():
      shape, approach = tup
      if shape == 'poly 10' and approach == 'geo3d' and timeStamp < datetime.datetime(year=2019, month=2, day=19):
        continue
  ...
  for key in 'poly 10', 'polyMedium', 'box':
...
with pysftp.Connection('people.apache.org', username='nknize', private_key='/home/nknize/.ssh/id_dsa') as c:
